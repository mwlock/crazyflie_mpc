...
def TRP2acc(TRP, psi, m, g = 9.81):
    ...
    phi = TRP[1]
    theta = TRP[2]
    ax = g * (sin(psi) * phi + cos(psi) * theta)
    ay = g * (-cos(psi) * phi + sin(psi) * theta)
    az = T / m
    return np.array([ax, ay, az])

def acc2TRP(a, psi, m, thrust_offset = 0.0, g = 9.81):
    """
    ...
    phi = (a[0] * sin(psi) - a[1] * cos(psi)) / g
    theta = (a[0] * cos(psi) + a[1] * sin(psi)) / g
